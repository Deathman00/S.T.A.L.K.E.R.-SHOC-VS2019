...
int do_list(uf) unzFile uf;
{
	...
	for (i = 0; i < gi.number_entry; i++)
	{
		...
		printf("%7lu  %6s%c%7lu %3lu%%  %2.2lu-%2.2lu-%2.2lu  %2.2lu:%2.2lu  %8.8lx   %s\n",
			   file_info.uncompressed_size, string_method, charCrypt, file_info.compressed_size, ratio,
...
char* argv[];
{
	...
	else if (opt_do_extract == 1)
	{
		...
		if (filename_to_extract == NULL)
			...
			return do_extract_onefile(uf, filename_to_extract, opt_do_extract_withoutpath, opt_overwrite, password);
	}
