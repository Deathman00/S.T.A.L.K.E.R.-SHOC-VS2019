...
BOOL CSoundRender_TargetD::_initialize()
{
	...
	dsBD.dwFlags = DSBCAPS_CTRL3D | DSBCAPS_CTRLFREQUENCY | DSBCAPS_CTRLVOLUME | DSBCAPS_GETCURRENTPOSITION2 |
...
void CSoundRender_TargetD::fill_parameters()
{
	...
	// 2. Set 2D params (volume, freq) + position(rewind)
	{
		...
		if (_abs(hw_freq - cache_hw_freq) > 50)
		{
			...
			clamp(hw_freq_set, s32(SoundRenderD->dsCaps.dwMinSecondarySampleRate),
				  s32(SoundRenderD->dsCaps.dwMaxSecondarySampleRate));
