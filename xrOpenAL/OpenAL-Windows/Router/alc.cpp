...
//*****************************************************************************
BOOL GetLoadedModuleDirectory(LPCTSTR moduleName, LPTSTR directoryContainingModule,
...
ALboolean FillOutAlcFunctions(ALCdevice* device)
{
	...
	alcFxns = (alcApi->alcCreateContext && alcApi->alcMakeContextCurrent && alcApi->alcProcessContext &&
			   ...
			   alcApi->alcIsExtensionPresent && alcApi->alcGetProcAddress && alcApi->alcGetEnumValue &&
			   alcApi->alcGetString && alcApi->alcGetIntegerv);
...
ALboolean FillOutAlFunctions(ALCcontext* context)
{
	...
	alFxns =
		(alApi->alEnable && alApi->alDisable && alApi->alIsEnabled &&
									...
									if (captureDevice == false)
									{
										...
										else
										{
											...
											if (device != NULL)
											{
												...
												{
													deviceSpecifier =
														alcGetStringFxn(device, ALC_DEFAULT_DEVICE_SPECIFIER);
													if (deviceSpecifier != NULL)
													{
														if ((partialName == false) ||
															(strstr(deviceSpecifier, "Generic") != 0))
...
//*****************************************************************************
//
ALCAPI ALCdevice* ALCAPIENTRY alcCaptureOpenDevice(const ALCchar* deviceName, ALCuint frequency, ALCenum format,
												   ALCsizei buffersize)
{
	char newDeviceName[256];
