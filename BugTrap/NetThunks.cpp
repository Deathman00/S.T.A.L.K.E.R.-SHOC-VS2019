...
static String ^
	GetMethodSignature(MethodBase ^ method, bool addType) {
	...
	static void GetStackTraceEntry(StackFrame ^ stackFrame, CNetStackTrace::CNetStackTraceEntry& rEntry)
{
	...
	{
		if (columnNumber != 0)
			swprintf_s(rEntry.m_szLineInfo, countof(rEntry.m_szLineInfo), L"line %lu, column %lu", lineNumber,
					   columnNumber);
		else
...
void CNetStackTrace::GetErrorString(CStrStream& rStream)
{
	...
	if (*ErrorInfo.m_szMessage &&
		wcsncmp(ErrorInfo.m_szMessage, wszStandardExceptionPrefix, dwStandardExceptionPrefixLength) != 0)
	{
		rStream << L" (";
