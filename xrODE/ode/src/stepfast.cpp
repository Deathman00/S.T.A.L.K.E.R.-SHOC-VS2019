...
void dInternalStepIslandFast(dxWorld* world, dxBody* const* bodies, int nb, dxJoint** joints, int nj, dReal stepsize,
							 int maxiterations)
{
#ifdef TIMING
		...
		for (j = 0; j < nj; j++)
		{
			...
			if (info[j].m > 0)
			{
				...
				case dJointTypeContact:
					if (info[j].m == 3)
					{
						dInternalStepJointContact(world, bodyPair, GIPair, GinvIPair, joint, info[j], Jinfo[j],
												  ministep);
